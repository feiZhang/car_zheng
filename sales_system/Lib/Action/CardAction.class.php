...
class CardAction extends DataOpeAction {
    ...
    public function save_topup(){
        ...
        if(isset($_REQUEST["topupMoney"])){
            ...
            //记录每次充值的信息
            $mTopup->add(array("customer_id"=>intval($_REQUEST["customer_id"]),
                'sales_price'=>$topupMoney,
                'finish_price'=>$topupMoney+$theCustomer["remaining_money"],
                'remaining_money'=>$topupMoney+$theCustomer["remaining_money"],
                'commodity_type'=>'用户充值',
                'descript'=>$_REQUEST["descript"]."[上次结余:".$theCustomer["remaining_money"].";充值后结余:".($topupMoney+$theCustomer["remaining_money"])."]"));
            //更新总计
        ...
        if(isset($_REQUEST["giftMoney"])){
            ...
            //记录每次充值的信息
            $mTopup->add(array("customer_id"=>intval($_REQUEST["customer_id"]),
                'sales_price'=>$giftMoney,
                'finish_price'=>$giftMoney+$theCustomer["remaining_money"],
                'remaining_money'=>$topupMoney+$theCustomer["remaining_money"],
                'commodity_type'=>'充值赠送',
                'descript'=>$_REQUEST["descript"]."[上次结余:".$theCustomer["remaining_money"].";赠送后结余:".($giftMoney+$theCustomer["remaining_money"])."]"));
            //更新总计
            $mCustomer->save(array("id"=>intval($_REQUEST["customer_id"]),
