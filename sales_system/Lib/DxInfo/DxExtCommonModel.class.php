...
class DxExtCommonModel extends Model {
	...
	* **/
	 ...
	 * 附加功能:自动生成数据查询列表，对DataOpe提供基础数据
	 ...
	 type:字段类型，分为：string[字符]、int、float（sigma支持这3种）、time[时间]、date[日期]、y_m(只显示年份和月份)、datetime[日期时间]、enum[枚举,单选框]、select[枚举,下拉框]、set[集合]、uploadFile[文件上传]、canton[区域]、cutPhoto[剪切头像]
	 	...
	 	canton的附属参数:"canton"=>array("rootCantonId"=>3520),
	 title:中文说明
	 ...
	 readOnly:字段数据为只读....类似HIDE，可以取值。READONLY_FIELD_ADD   READONLY_FIELD_EDIT
	 		一种情况：字典表的维护，添加时需要类型显示为不能改，但是又需要将类型值追加到数据中，则再model中重新定义save方法，如果是新增，则将type字段readOnly改为false,
	 		注：readOnly的字段，还是会从前端post过来，但是在后端会被忽略掉
	...
	* **/
	...
	private function getOneListField($key,$field){
		...
		if(empty($field["field_enum"]) && array_key_exists("type", $field) && ($field["type"]=="enum" || $field["type"]=="set" || $field["type"]=="select")){
			if(isset($field["valChange"])){
				...
				if(!empty($tField)){
					//枚举类型的内部序号是从1开始。
					$tttt	= explode(",","0,".str_replace(array("'","(",")"),array("","",""), substr($tField[0]["COLUMN_TYPE"],5)));
					unset($tttt[0]);$field_enum	= array();
					foreach($tttt as $key=>$val){
						$field_enum[$val]	= $val;
					}
					$field["field_enum"]	= $field_enum;
				}
			}
		}
		...
		foreach($this->getListFields() as $name=>$field){
			...
			if(!($field["hide"] & self::HIDE_FIELD_ADD) && $pkId==0){
				...
				if($field["readOnly"]===true || $field["readOnly"]===false)
					$f[$field["name"]]["readOnly"]	= $field["readOnly"];
				else
	...
	protected function _options_filter(&$options) {
		...
		if($this->skipDataPowerCheck || checkInNotArray(C('DP_NOT_CHECK_MODEL'),array(),$this->name)) return;
		...
		if(is_array(C('DP_POWER_FIELDS')) && sizeof(C('DP_POWER_FIELDS'))>0 && (!array_key_exists("DP_ADMIN", $_SESSION) || !$_SESSION["DP_ADMIN"])){        //为了提高代码执行效率
			...
			if(!checkNotAuth(C('DP_NOT_CHECK_ACTION'))){
	 ...
	 * @return string
	 */
	public function getDeleteStateWhere($array = true,$table_alias = ""){
		$dbFields       		= $this->getDbFields();
		$dataPowerFieldDelete	= "";
		//if(APP_DEBUG) Log::write(var_export($dbFields,true).MODULE_NAME."|".ACTION_NAME."__dbFields",Log::INFO);
		//追加数据删除字段标志,,直接追加Where条件。
